...
import { getModel, MIN_TRAINING_SAMPLES } from '../mlScoring.js';
...
export const underwritingAgent: AgentDefinition = {
  ...
  tools: [
    ...
    },
    {
      ...
      description:
        'Probabilidade de resultado ruim (sinistro aprovado ou necessidade de recuperação jurídica). Usa o modelo de regressão logística real treinado sobre o histórico da Lastro quando há amostras suficientes; caso contrário usa uma premissa assumida por rating (lib/defaultProbability.ts), sempre sinalizada como tal — nunca apresentada como o modelo treinado.',
      ...
      handler: async (input: { duplicataId: string }) => {
        ...
        if (!model) {
          return {
            disponivel: true,
            fonte: 'assumida',
            probabilidadeResultadoRuim: estimate.pd,
            motivo: `Modelo de ML ainda não treinado (mínimo de ${MIN_TRAINING_SAMPLES} amostras rotuladas na base) — probabilidade é uma premissa assumida por rating (${estimate.rating}), não uma previsão do modelo.`,
          };
        }
        return {
          disponivel: true,
          fonte: 'ml',
          probabilidadeResultadoRuim: estimate.pd,
          amostrasDeTreino: model.nSamples,
          acuraciaDeTreino: model.trainAccuracy,
          treinadoEm: model.trainedAt,
        };
      },
    },
  ],
