-- Adds the self-service, no-KYB 'api_partner' role (Score API / PLD Screening API as a
-- standalone product — see README "Score/PLD API como produto standalone"). Same table-
-- recreation pattern as 0045_auditor_role.sql for widening a CHECK constraint in SQLite —
-- every column below copied verbatim from 0045's schema (nothing added to `users` since).
CREATE TABLE users_new_0049 (
  ...
  role TEXT NOT NULL CHECK(role IN ('investidor','cedente','sacado','admin','seguradora','auditor','api_partner')),
...
DROP TABLE users;
ALTER TABLE users_new_0049 RENAME TO users;
