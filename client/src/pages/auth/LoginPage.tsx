...
const SAML_ERROR_LABELS: Record<string, string> = {
  nao_configurado: 'SSO corporativo não está configurado neste ambiente.',
  sessao_invalida: 'Sessão de login via SSO expirada — tente novamente.',
...
export function LoginPage() {
  ...
  const continueWithSaml = () => {
    const ref = new URLSearchParams(window.location.search).get('ref');
    window.location.href = `/api/auth/saml/login${ref ? `?ref=${encodeURIComponent(ref)}` : ''}`;
  };
