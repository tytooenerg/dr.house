...
// only collects what Google can't provide: role and company name. No password field —
// the account is Google-only unless the user later sets one from Perfil.
export function CompleteGoogleSignupPage() {
  const { user, completeGoogleSignup, authError } = useSession();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await completeGoogleSignup({ signupToken, companyName, role, insurerKey: role === 'seguradora' ? insurerKey! : undefined });
    } catch {
