...
export function EmitirPage() {
  ...
  const [result, setResult] = useState<{ registro: string; seguro: boolean } | null>(null);
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const data = await api.post<{ registro: string; seguro: boolean }>('/emitir/submit', { ...form, nfAnexada, batchValores: batchRows.map((r) => r.valor) });
      setResult(data);
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Falha ao registrar na CERC — conexão instável. Tente novamente.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div>
        ...
        <Card className="p-10 text-center max-w-[560px]">
          ...
          <div className="text-textSecondary text-[13.5px] mt-2">Registro escritural nº {result.registro} confirmado na CERC. Status: enviada ao Marketplace.</div>
      ...
      <div className="grid gap-4 items-start" style={{ gridTemplateColumns: '1.4fr 1fr' }}>
        <Card className="p-7 flex flex-col gap-4">
          ...
          <Button disabled={submitting} onClick={submit} className="py-3.5">
            {submitting ? 'Registrando na CERC…' : 'Emitir e registrar duplicata escritural'}
          </Button>
