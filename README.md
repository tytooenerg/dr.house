...
- **Admin / back-office role** — a fourth role (`admin`) with its own screen: a KYB approval queue (approve/reject institutional investors, with a reason on rejection), dispute arbitration (decide for cedente or sacado), a Fila de Compliance (duplicatas the Compliance AI Engine suspended for review — see "Claude-assisted features" below), and a Uso de IA panel (real per-feature Claude call/token/cost metering — see "Platform hardening" below). Investors can't bid until their KYB is approved.
