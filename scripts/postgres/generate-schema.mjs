...
function translate(sql) {
  return sql
    ...
    // strftime('%Y-%m-%dT%H:%M:%fZ', 'now'[, '+N unit']) — o jeito de o SQLite escrever um
    // instante ISO-8601 com 'T'/'Z', que é o formato em que close_at/created_at são gravados
    // e COMPARADOS COMO STRING no código (listAuctionsToClose). datetime() sozinho não serve:
    // ele produz "2026-09-05 12:00:00", e o espaço ordena antes do 'T' do mesmo dia. Postgres
    // escreve o mesmo instante com to_char + interval. Terceira regra desde que este script
    // nasceu, e pelo mesmo motivo das outras duas: uma migração real precisou dela.
    .replace(
      /strftime\('%Y-%m-%dT%H:%M:%fZ',\s*'now'(?:,\s*'([+-]\d+ \w+)')?\)/g,
      (_m, delta) =>
        `to_char(now()${delta ? ` + interval '${delta}'` : ''}, 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')`
    );
}
